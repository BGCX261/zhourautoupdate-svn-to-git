...
const
  NSUBEXP = 100; // max number of subexpression //###0.929
...
{$ENDIF}
    ...
    {===================== Mathing section ===================}
    function regrepeat(p: PRegExprChar; AMax: integer; out ANumLen: integer): integer;
...
function StrPCopy(ADest: PRegExprChar; const ASource: RegExprString): PRegExprChar;
var
  i, Len: Integer;
begin
  Len := length(ASource); //###0.932
  for i := 1 to Len do
    ADest[i - 1] := ASource[i];
  ADest[Len] := #0;
  Result := ADest;
...
function StrLCopy(ADest, ASource: PRegExprChar; MaxLen: Cardinal): PRegExprChar;
var i: Integer;
begin
...
{$ELSE}
{
 ...
 RusRangeHiHigh = '?;
  ...
  RusRangeLo: array[0..33] of REChar = (
    #$E0, #$E1, #$E2, #$E3, #$E4, #$E5, #$B8, #$E6, #$E7, #$E8, #$E9, #$EA, #$EB, #$EC, #$ED, #$EE,
    #$EF, #$F0, #$F1, #$F2, #$F3, #$F4, #$F5, #$F6, #$F7, #$F8, #$F9, #$FA, #$FB, #$FC, #$FD, #$FE,
    #$FF, #0
    );
  RusRangeHi: array[0..33] of REChar = (
    #$C0, #$C1, #$C2, #$C3, #$C4, #$C5, #$A8, #$C6, #$C7, #$C8, #$C9, #$CA, #$CB, #$CC, #$CD, #$CE,
...
{$ENDIF}
    ...
    if PREOp(regnext(scan))^ = EEND then begin // Only one top-ALevel choice.
...
// Combining parenthesis handling with the base ALevel of regular expression
...
function TRegExpr.regrepeat(p: PRegExprChar; AMax: integer; out ANumLen: integer): integer;
// repeatedly match something simple, report how many
var
  scan: PRegExprChar;
  opnd: PRegExprChar;
...
{$ENDIF}
      STAR, PLUS, BRACES, STARNG, PLUSNG, BRACESNG: begin
          ...
          if (scan^ = PLUSNG) or (scan^ = STARNG) or (scan^ = BRACESNG) then begin
             // non-greedy mode
            BracesMax := regrepeat(opnd, BracesMax, iNumLen); // don't repeat more than BracesMax
...
{$ENDIF}
            ...
            savenum := reginput;
            no := regrepeat(opnd, BracesMax, iNumLen); // don't repeat more than max_cnt
            if (iNumLen > 0) and SameText(StrPas(reginput), '') then
            begin
              for I := 1 to Length(startp) - 1 do
                if startp[I] = nil then
                begin
                  startp[I] := savenum;
                  endp[I] := reginput;
                  Break;
